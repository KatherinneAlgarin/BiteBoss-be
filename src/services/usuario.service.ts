...
export class UsuarioService {
  ...
  async listarUsuarios(): Promise<UsuarioListItem[]> {
  ...
  async crearUsuario(dto: CrearUsuarioDto): Promise<UsuarioCreadoResponse> {
    ...
    // Crear usuario en Supabase Auth (maneja el hash de la contraseña)
    ...
    if (usuarioError || !usuarioData) {
      // Revertir usuario auth creado para no dejar estado inconsistente
      await supabase.auth.admin.deleteUser(authData.user.id);
      throw new AppError('Error al registrar el usuario', 500);
    }

    // Insertar en tabla usuario_sucursal (vinculación con sucursal y rol)
    ...
    // Obtener nombres de rol y sucursal para la respuesta
    ...
    return {
      ...
      nombre,
      email,
      rol:      (rolData as any)?.nombre ?? '',
      sucursal: (sucursalData as any)?.nombre ?? '',
    };
  }
}
